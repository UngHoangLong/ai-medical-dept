...
logger = logging.getLogger("voxtell.modal.server")

app = FastAPI(title="VoxTell Modal API", version="0.1.0")
...
DEFAULT_VOXTELL_MODEL_DIR = "/app/models/voxtell_v1.1"
...
async def get_voxtell_volume(pid: str, series_uid: str):
    ...
    logger.info(
        "Prepared VoxTell volume for pid=%s series=%s from key=%s",
        pid,
        series_uid,
        s3_key,
    )
